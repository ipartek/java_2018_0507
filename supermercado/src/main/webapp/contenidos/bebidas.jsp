<html>

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel=stylesheet href="../css/bootstrap.min.css">
    <link rel=stylesheet href="../css/estilos.css">
    <link rel="stylesheet" href="css/fonts.css" />
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.3.1/css/all.css" integrity="sha384-mzrmE5qonljUremFsqc01SB46JvROS7bZs3IO2EmfFsd15uHvIt+Y8vEf7N7fWAU" crossorigin="anonymous">
    <link rel="shortcut icon" href="../imgs/favicon.ico" />
    <title>Supermercado Ana</title>
</head>

<body>
  
    <div class="jumbotron">
      <h1>Supermercado Ana</h1> 
    </div>
  
    <jsp:include page="menuprincipal.jsp" />
    
	  <div id="contenedor">
      
		  <div class="row">
			<div class="col-sm-3">
				  <div class="bebida">
				 	<img class="bebidafoto" src="../imgs/bebidas/kas.jpg" alt="Ron"/> 
				  </div>
				  <div class="bebida">
				 		<div class="preciotachado">30€</div>
				        <div class="preciooferta">20€ </div>
				        <button type="button" class="btn btn-success"><i class="fas fa-shopping-cart"></i></button>  
				  </div>
				   <div class="bebida">
				 	<img class="bebidafoto" src="../imgs/bebidas/mosto.jpg" alt="Cola"/> 
				  </div>
				  <div class="bebida">
				 		<div class="preciotachado">30€</div>
				        <div class="preciooferta">20€ </div>
				        <button type="button" class="btn btn-success"><i class="fas fa-shopping-cart"></i></button>  
				  </div>
				 
			  </div>
			  <div class="col-sm-3">
				  <div class="bebida">
				 	<img class="bebidafoto" src="../imgs/bebidas/cola.jpg" alt="Ron"/> 
				  </div>
				  <div class="bebida">
				 		<div class="preciotachado">30€</div>
				        <div class="preciooferta">20€ </div>
				        <button type="button" class="btn btn-success"><i class="fas fa-shopping-cart"></i></button>  
				  </div>
				   <div class="bebida">
				 	<img class="bebidafoto" src="../imgs/bebidas/ron.jpg" alt="Cola"/> 
				  </div>
				  <div class="bebida">
				 		<div class="preciotachado">30€</div>
				        <div class="preciooferta">20€ </div>
				        <button type="button" class="btn btn-success"><i class="fas fa-shopping-cart"></i></button>  
				  </div>
			  </div>
			  <div class="col-sm-3">
				  <div class="bebida">
				 	<img class="bebidafoto" src="../imgs/bebidas/tequila.jpg" alt="Ron"/> 
				  </div>
				  <div class="bebida">
				 		<div class="preciotachado">30€</div>
				        <div class="preciooferta">20€ </div>
				        <button type="button" class="btn btn-success"><i class="fas fa-shopping-cart"></i></button>  
				  </div>
				   <div class="bebida">
				 	<img class="bebidafoto" src="../imgs/bebidas/mosto.jpg" alt="Cola"/> 
				  </div>
				  <div class="bebida">
				 		<div class="preciotachado">30€</div>
				        <div class="preciooferta">20€ </div>
				        <button type="button" class="btn btn-success"><i class="fas fa-shopping-cart"></i></button>  
				  </div>
			  </div>
			  <div class="col-sm-3">
				  <div class="bebida">
				 	<img class="bebidafoto" src="../imgs/bebidas/kas.jpg" alt="Ron"/> 
				  </div>
				  <div class="bebida">
				 		<div class="preciotachado">30€</div>
				        <div class="preciooferta">20€ </div>
				        <button type="button" class="btn btn-success"><i class="fas fa-shopping-cart"></i></button>  
				  </div>
				   <div class="bebida">
				 	<img class="bebidafoto" src="../imgs/bebidas/tequila.jpg" alt="Cola"/> 
				  </div>
				  <div class="bebida">
				 		<div class="preciotachado">30€</div>
				        <div class="preciooferta">20€ </div>
				        <button type="button" class="btn btn-success"><i class="fas fa-shopping-cart"></i></button>  
				  </div>
			  </div>
		</div>
	
</body>

<footer>
  <div id="pie">
       <i class="fas fa-envelope"></i>
        <p>Mail <a href="mailto:mail@mail.com"> mail@mail.com</a>.</p>
       <div id="iconredsocial">
           <img class="redsocial" src="../imgs/facebook.png" width="30px">
           <img class="redsocial" src="../imgs/twiter.png" width="30px">
           <img class="redsocial" src="../imgs/insta.jpg" width="30px">
       </div>
       <div >&copy;</div>
       <a href="politicapriva.jsp">Politica de privacidad - </a>
       <a href="contacto.jsp">Contacto - </a>
       <a href="localizacion.jsp">Localizacion</a>
   
   </div>
</footer>

</html>