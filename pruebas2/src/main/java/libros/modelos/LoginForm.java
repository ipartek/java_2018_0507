...
public class LoginForm {
	...
	public void setNombre(String nombre) { //throws LoginFormException { // "              Hola Javier            ".trim() ---> "Hola Javier"
		if(nombre == null || nombre.trim().length() == 0) {
			setErrorNombre("No se admiten nombres vac�os");
			setErroneo(true);
			throw new LoginFormException("No se admiten nombres vac�os");
		}
		this.nombre = nombre;
	}
	...
	public void setPassword(String password) {
		if(password == null || password.trim().length() == 0) {
			setErrorPassword("No se admiten nombres vac�os");
			setErroneo(true);
			throw new LoginFormException("No se admiten passwords vac�os");
		}
